...
| Member | Lane |
|---|---|
| Meherab (meherabhossainmmh-lang) — **team lead** | Overall architecture · kinematics core & motion pipeline · 3D rendering & robot model · frontend↔backend wiring · README/docs · release |
| alaminakash550 | Manual-control UI & inputs · ESP32/servo hardware & schematics |
| oishesalma | Backend: accounts & authentication (`server/src/routes/auth.ts`, JWT/bcrypt) · Prisma schema · `AuthPanel` |
| akterjobaida57-debug | Backend: persistent event log (`server/src/routes/events.ts`) · `HistoryPanel` · voice control · agentic AI layer |
