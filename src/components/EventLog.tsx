...
const LEVEL_TINT = {
  ...
  warn: 'text-alarm',
  error: 'text-alarm',
  security: 'text-flare', // Added security level tint
} as const;
...
const SOURCE_TINT: Record<string, string> = {
  ...
  auto: 'text-flare',
  system: 'text-dim',
  security: 'text-alarm', // Added security source tint
};
...
export default function EventLog() {
  const storeEvents = useArmStore((s) => s.events);
  const { user } = useAuthStore();
  const isAdmin = user?.role === 'ADMIN';

  // Filter out security events for non-admins to protect privacy/passwords
  const events = storeEvents.filter(e => isAdmin || (e.source !== 'security' && e.level !== 'security'));
  
  const refused = events.filter((e) => e.level !== 'info' && e.level !== 'security').length;
