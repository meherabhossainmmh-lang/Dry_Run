...
/**
 * DELETE /api/events
 * Clears the caller's own saved command history (registered operators only).
 * Scoped to the authenticated user, so every operator — admin or not — clears
 * only their own events, the same per-user slice the panel displays for them.
...
router.delete('/', requireAuth, async (req: AuthedRequest, res) => {
  ...
  const result = await prisma.event.deleteMany({ where: { userId } });
